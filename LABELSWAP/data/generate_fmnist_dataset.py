...
class FMNISTGenerate:
    ...
    def _load_MNIST(self, train=True):
        if train:
            mnist_dataset = datasets.FashionMNIST(root='./data/', train=True, transform=transforms.ToTensor(), download=True)
        else:
            mnist_dataset =datasets.FashionMNIST(root='./data/', train=False, transform=transforms.ToTensor(), download=True)

        dl = DataLoader(mnist_dataset)

        X = dl.dataset.data # (50000,28, 28), ([10000, 28, 28])
        y = dl.dataset.targets #(50000)
