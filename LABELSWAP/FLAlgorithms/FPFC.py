...
class FPFC(Base):
    def __init__(self, batch_size, learning_rate, lamda, a, xi, num_glob_iters, local_epochs, num_users, ratio, dimension, class_num, seed, projectdir, datasetdir):
        ...
        self.output_dir = os.path.join(projectdir, 'results_FPFC_'+str(lamda)+'.pickle')
    ...
    def run(self):
        ...
        print('*'*50)
        print('start training FPFC with lamda: ', lamda)
        for glob_iter in range(self.num_glob_iters):
            ...
            if glob_iter == 0:
                ...
                theta, V = self.server(V = V, lam1 = xi, lam = lamda, a = a, rho = 1, participating_clients = np.arange(NUM_USER))
            ...
            theta, V = self.server(V = V, lam1 = xi, lam = lamda, a = a, rho = 1, participating_clients = participating_clients) 
        ...
        print("---train FPFC Ended in %0.2f hour (%.3f sec) " % (duration/float(3600), duration)) 
    ...
    def server(self, V, lam1, lam, a, rho, participating_clients):
        NUM_USER = self.num_users
        NUM_CLASS = self.classnum
        dimension = self.dimension
        ...
        for m_i in participating_clients: 
            ...
            for m_j in range(NUM_USER):
                ...
                if torch.norm(delta, p = 'fro') <= (lam1 + lam/rho):
                    theta[m_i][m_j] = lam1*rho/(lam + lam1*rho) * delta
                elif (lam1 + lam/rho) <= torch.norm(delta, p = 'fro') <= (lam + lam/rho):
                    theta[m_i][m_j] = (1 - lam/(rho *torch.norm(delta, p = 'fro')))*delta 
                elif lam + lam/rho < torch.norm(delta, p = 'fro') <= a * lam:
                    theta[m_i][m_j] = softThresholding(z = delta, t = a*lam/((a-1)*rho))/(1 - 1/((a-1)*rho))
                elif torch.norm(delta, p = 'fro')> a*lam:
                    theta[m_i][m_j] = delta
