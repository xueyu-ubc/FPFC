...
def calculate_loss_grad(model, lr, criterion, X, y):
    y_target = model(X)
    y = y.long()
    ...
    n_weight = weight - lr * d_weight
    n_bias = bias - lr * d_bias
    
    return loss_value, n_weight, n_bias

def weight_update(scores, weights):
    m = len(weights)
    tmp = torch.zeros_like(weights[0])
    for m_i in range(m):
        tmp += scores[m_i] * weights[m_i]
    tmp /= m

    return tmp


def grads_update(scores, grads):
    m = len(grads)
...
def threshold(tobeselected_users, dataset, theta_norm2, coef, value):
    cluster = np.zeros(len(tobeselected_users))+100
    tmp = []
    for i in tobeselected_users:
        tmp_i = []
        for j in tobeselected_users:
            if j >= i and theta_norm2[i][j] < value:
                tmp_i.append(j)
            elif j<i and theta_norm2[j][i] < value:
                tmp_i.append(j)
        tmp.append(tmp_i)
    ...
    for id in tt:
        coef_avg = torch.zeros_like(coef[0])
        num_samples = 0
        for i in id:
            user_sample = dataset['data'][i][1].shape[0]
            coef_avg += coef[i] * user_sample
            num_samples += user_sample
    
        coef_avg = coef_avg/num_samples
