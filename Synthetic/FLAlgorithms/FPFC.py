...
class FPFC(Base):
    def __init__(self, batch_size, learning_rate, lamda, a, xi, threshold, num_glob_iters, local_epochs, num_users, ratio, dimension, class_num, seed, projectdir, datasetdir):
    ...
    def run(self):
        ...
        print('*'*50)
        print('start training FPFC with lamda: ', lamda, ' and threshold: ', self.threshold)
        for glob_iter in range(self.num_glob_iters):
            ...
            for m_i in selected_users:
                ...
                for m_j in range(NUM_USER):
                    ...
                    if torch.norm(delta, p = 'fro') <= (xi + lamda/rho):
                        theta[m_i][m_j] = xi*rho/(lamda + xi*rho) * delta
                    elif (xi + lamda/rho) <= torch.norm(delta, p = 'fro') <= (lamda + lamda/rho):
                        theta[m_i][m_j] = (1 - lamda/(rho *torch.norm(delta, p = 'fro')))*delta 
                    elif lamda + lamda/rho < torch.norm(delta, p = 'fro') <= a * lamda:
                        theta[m_i][m_j] = softThresholding(z = delta, t = a*lamda/((a-1)*rho))/(1 - 1/((a-1)*rho))
                    elif torch.norm(delta, p = 'fro')> a*lamda:
                        theta[m_i][m_j] = delta
